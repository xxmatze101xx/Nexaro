    ...
    <div className="hidden lg:flex items-center justify-center flex-1 px-4">
      <div className="relative w-full h-px">
        ...
        <motion.div
          ...
          style={{ background: "linear-gradient(90deg, #7B68EE, #818CF8)" }}
          initial={{ width: "0%" }}
          animate={active ? { width: "100%" } : { width: "0%" }}
          transition={{ duration: 0.8, delay: 0.5, ease: "easeInOut" }}
        />
        <div className="absolute right-0 top-1/2 -translate-y-1/2 w-2 h-2 rounded-full bg-[#818CF8]" />
      </div>
    </div>
  );
}
...
export function HowItWorksSection() {
  ...
  return (
    <section
      ref={ref}
      id="how-it-works"
      className="relative bg-zinc-50 dark:bg-zinc-900/40 py-20 sm:py-28 lg:py-36 px-4 sm:px-6 overflow-hidden transition-colors duration-300"
    >
      ...
      <div
        ...
        style={{
          backgroundImage: `linear-gradient(rgba(100,100,100,0.8) 1px, transparent 1px), linear-gradient(90deg, rgba(100,100,100,0.8) 1px, transparent 1px)`,
      ...
      <div className="relative max-w-6xl mx-auto">
        ...
        >
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#7B68EE] mb-3">How It Works</p>
          ...
          {STEPS.map((step, i) => (
            <div key={step.number} className="flex flex-col lg:flex-row flex-1 items-stretch">
              <motion.div
                ...
                className="flex-1 relative flex flex-col gap-5 p-7 sm:p-8 rounded-2xl border border-zinc-200 dark:border-white/[0.07] bg-white dark:bg-zinc-950/80 overflow-hidden"
              >
                {/* Step number background watermark */}
                <div
                  className="absolute top-4 right-5 text-7xl font-black leading-none select-none pointer-events-none text-zinc-100 dark:text-white/[0.04]"
                >
                  {step.number}
                </div>
                {/* Icon */}
                ...
                {/* Step label */}
                <span className="text-xs font-bold uppercase tracking-[0.14em]" style={{ color: step.color }}>
                  Step {step.number}
                </span>
                {/* Title */}
                ...
                <div className="pt-4 border-t border-zinc-100 dark:border-white/[0.06]">
                  <p className="text-xs text-zinc-400 dark:text-zinc-600 font-medium">{step.detail}</p>
                </div>
              </motion.div>
        ...
        >
          <div className="inline-flex items-center gap-3 rounded-2xl border border-[#7B68EE]/20 bg-violet-50 dark:bg-[#7B68EE]/[0.06] px-6 py-4">
            <span className="text-2xl font-bold text-[#7B68EE] dark:text-[#9B8AFE]">&lt;10 min</span>
            <div className="text-left">
              <p className="text-zinc-900 dark:text-white font-semibold text-sm">Time to first insight</p>
              <p className="text-zinc-500 dark:text-zinc-500 text-xs">From signup to your first AI briefing</p>
            </div>
          </div>
          <div className="inline-flex items-center gap-3 rounded-2xl border border-zinc-200 dark:border-white/[0.07] bg-white dark:bg-white/[0.025] px-6 py-4">
            <span className="text-2xl font-bold text-zinc-700 dark:text-zinc-300">Zero</span>
            <div className="text-left">
              <p className="text-zinc-900 dark:text-white font-semibold text-sm">Engineering required</p>
              <p className="text-zinc-500 dark:text-zinc-500 text-xs">Self-serve, OAuth-based, no IT ticket</p>
            </div>
          </div>
        </motion.div>
      </div>
