...
function TestimonialCard({ testimonial, index }: { testimonial: Testimonial; index: number }) {
  ...
  const highlightText = (text: string, highlight?: string) => {
    ...
    return (
      <>
        {parts[0]}
        <span className="text-zinc-900 dark:text-white font-semibold">{highlight}</span>
        {parts[1]}
      </>
    );
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 pt-4 border-t border-zinc-100 dark:border-white/[0.06]">
        <div className={cn("w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0", testimonial.color)}>
          {testimonial.initials}
        </div>
        <div>
          <p className="text-zinc-900 dark:text-white font-semibold text-sm">{testimonial.name}</p>
          <p className="text-zinc-500 dark:text-zinc-500 text-xs">
            {testimonial.title} · {testimonial.company}
          </p>
        </div>
        <div className="ml-auto text-yellow-500 dark:text-yellow-400 text-sm tracking-tighter">★★★★★</div>
      </div>
    </motion.div>
  );
}
    ...
    <section ref={ref} className="relative bg-zinc-50 dark:bg-zinc-950 py-20 sm:py-28 px-4 sm:px-6 overflow-hidden transition-colors duration-300">
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <motion.div
          ...
          className="mt-12 flex flex-wrap items-center justify-center gap-6 text-sm text-zinc-500 dark:text-zinc-500"
        >
          <div className="flex items-center gap-2">
            <span className="text-yellow-500 dark:text-yellow-400 text-base">★★★★★</span>
            <span><strong className="text-zinc-900 dark:text-white">4.9/5</strong> on G2</span>
          </div>
          <div className="w-px h-4 bg-zinc-200 dark:bg-zinc-800" />
          <div className="flex items-center gap-2">
            <span className="text-yellow-500 dark:text-yellow-400 text-base">★★★★★</span>
            <span><strong className="text-zinc-900 dark:text-white">4.8/5</strong> on Product Hunt</span>
          </div>
          <div className="w-px h-4 bg-zinc-200 dark:bg-zinc-800" />
          <span><strong className="text-zinc-900 dark:text-white">500+</strong> executive teams worldwide</span>
        </motion.div>
      </div>
    </section>
  );
