 ...
 * Body: { email: string }
 *
 * Registers (or renews) a Gmail push watch via Google Cloud Pub/Sub.
 * Stores pushActive + watchExpiration in the user's Gmail sync state.
 * Writes an email → uid index at Firestore: emailIndex/{encodedEmail}.
 *
 * Requires env vars:
 *   GMAIL_PUBSUB_TOPIC  — full Pub/Sub topic name, e.g. projects/my-project/topics/nexaro-gmail-push
...
async function getRefreshToken(uid: string, email: string, idToken: string): Promise<string | null> {
    ...
    if (!res.ok) return null;
    const doc = (await res.json()) as { fields?: { accounts?: { arrayValue?: { values?: Array<{ mapValue?: { fields?: Record<string, { stringValue?: string }> } }> } } } };
...
/** Encode email into a Firestore-safe document ID. */
...
export async function POST(request: Request) {
    ...
    try {
        ...
        return NextResponse.json({ pushActive: true, expiration });
    } catch (err: unknown) {
