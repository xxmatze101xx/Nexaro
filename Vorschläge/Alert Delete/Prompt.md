...
- **Placement**: Place the component in `src/components/ui/alert-dialog.tsx`.
- **Usage**: Integrate this as a generic UI component for deletion alerts. Usage examples include `src/components/message-card.tsx` (when deleting emails) and `src/components/files-drive.tsx` (when deleting files).
...
function AlertDialogWithIconDemo() {
  return (
    <AlertDialog>
      <AlertDialogTrigger asChild>
...
export { AlertDialogDemo, AlertDialogWithIconDemo }
```
